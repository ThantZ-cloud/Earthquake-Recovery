...
export default function AdminMonitoring() {
  const { data: locations = [], isLoading: locsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('locations')
        .select('id, user_id, latitude, longitude, label, created_at');
  ...
  const { data: earthquakes = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        'https://www.seismicportal.eu/fdsnws/event/1/query?format=json&minmag=3&limit=50&orderby=time'
      );
  ...
  const nearEpicenter = locations.filter((loc) =>
    earthquakes.some(
      (eq) => haversineKm([loc.latitude, loc.longitude], [eq.lat, eq.lon]) < RADIUS_KM
    )
  );
    ...
    <Box>
      ...
      {nearEpicenter.length > 0 && (
        <Alert severity="warning" sx={{ mb: 2, borderRadius: 2 }}>
          {nearEpicenter.length} user location(s) within {RADIUS_KM} km of a recent M3+
          epicenter.
        </Alert>
      )}
          ...
          >
            ...
            {earthquakes.map((eq, i) => (
              <CircleMarker
                ...
                pathOptions={{ color: '#d32f2f', fillColor: '#ef5350', fillOpacity: 0.6 }}
              >
          ...
          <CardContent>
            ...
            {locations.length === 0 ? (
              <Typography variant="body2" color="text.secondary">
                No users have saved locations yet.
              </Typography>
            ) : (
